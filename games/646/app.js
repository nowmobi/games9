...
EventHandlers.resizeContent = function() {
    ...
    Main.bgImageBottom.scale.set(bgScale);
    Main.bgImageBottom.y = Main.height / 2;
...
Game.prototype.create = function(_managers, _numLevels, _numImages) {
    ...
    var firstPicks = [4, 15, 16, 18, 19, 20, 21];
    var featured = 2;
    var firstOrder = [];
    for (var i = 0; i < firstPicks.length; i++) firstOrder[i] = firstPicks[i] - 1;
    Utils.shuffleList(firstOrder);
    firstOrder.splice(featured, firstPicks.length - featured);
    var restOrder = [];
    var j = 0;
    for (var i = 0; i < this.numImages; i++) {
        if (firstOrder.indexOf(i) == -1) restOrder[j++] = i
    }
    Utils.shuffleList(restOrder);
    var order = firstOrder.concat(restOrder);
    this.levelOrder = [];
...
Main.prototype.createGame = function() {
    ...
    Main.bgImageBottom = new PIXI.Sprite;
    Main.bgImageBottom.anchor.set(.5, 1);
    Main.stage.addChild(Main.bgImageBottom);
...
Titles.prototype.setOrientationParams = function() {
    ...
    if (Main.aspectRatio > .9 && Main.aspectRatio < 1.15) {
        ...
        this.startButtonScale = 1;
        if (Main.aspectRatio >= 2.4) {
            this.showLogoScale = .45;
            this.showTitleScale = .3;
            this.gameTitleY = 0;
            ...
            this.showLogoScale = .75;
            this.showTitleScale = .65;
            this.gameTitleY = -20;
            this.startButtonY = 400;
            this.titleGap = 200;
            this.characterScale = 1.3
...
Titles.prototype.create = function(_preloader) {
    ...
    this.bg = new Sprite;
    this.bg.create(Main.bgImageBottom, "game_title_bg", this.managers.textures);
    this.bg.anchor.set(.5, 1);
    ...
    this.characterSpriteRight.create(Main.rightBottomUI, "game_title_characters_right", this.managers.textures, 0, 0);
    ...
    var _that = this;
    this.characterSpriteRight.x = this.characterSpriteRight.width / 2;
...
Preloader.prototype.preloaderReady = function() {
    ...
    this.textureManager.addImage("game_title_characters_right", imageAssetPath + "sprites/char_right.png");
