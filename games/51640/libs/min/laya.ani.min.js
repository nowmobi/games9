!function(t, e) {
        ...
        static parse(t, n) {
            ...
            t._anis.length = g, h = 0; h < g; h++) {
                ...
                for (v.totalKeyframeDatasLength = 0, l = 0; l < A; l++) {
                    ...
                    T.childs = [];
                    var S = n.getInt16();
                    S >= 0 && (T.name = y[S], v.bone3DMap[T.name] = l), T.keyFrame = [], T.parentIndex = n.getInt16(), 
                    -1 == T.parentIndex ? T.parent = null : T.parent = v.nodes[T.parentIndex], T.lerpType = n.getUint8();
                    var b = n.getUint32();
                    D.pos = b;
                    var C = T.keyframeWidth = D.getUint16();
                    if (v.totalKeyframeDatasLength += C, 0 === T.lerpType || 1 === T.lerpType) for (T.interpolationMethod = [], 
                    T.interpolationMethod.length = C, o = 0; o < C; o++) T.interpolationMethod[o] = i.AnimationTemplet.interpolation[D.getUint8()];
            ...
            for (h._templet._anis.length = d, t = 0; t < d; t++) {
                ...
                for (c.totalKeyframeDatasLength = 0, e = 0; e < x; e++) {
                    ...
                    for (n = 0, l = f; n < l; n++) {
                        ...
                        var v = h._DATA.offset, I = o.getUint32(), A = 4 * _, T = u.slice(v + I, v + I + A);
    ...
    class A {
        ...
        _applyIk2(t, i, a, r, s, n) {
            if (0 != n) {
                ...
                var S, b, C = 1 / (f * (v = T.resultMatrix.d) - M * D), F = a - T.resultMatrix.tx, w = r - T.resultMatrix.ty, k = (F * v - w * M) * C - u, P = (w * f - F * D) * C - _, L = ((F = x - T.resultMatrix.tx) * v - (w = y - T.resultMatrix.ty) * M) * C - u, U = (w * f - F * D) * C - _, B = Math.sqrt(L * L + U * U), E = i.length * c;
                if (I) {
                    var R = (k * k + P * P - B * B - (E *= p) * E) / (2 * B * E);
                    R < -1 ? R = -1 : R > 1 && (R = 1), b = Math.acos(R) * s, f = B + E * R, M = E * Math.sin(b), 
                    S = Math.atan2(P * f - k * M, k * f + P * M);
                } else {
                    ...
                    Q = v, Z = F, j = w), v > $ && (J = it, $ = v, tt = F, et = w), V <= (Q + $) / 2 ? (S = Y - Math.atan2(j * s, Z), 
                    b = H * s) : (S = Y - Math.atan2(et * s, tt), b = J * s);
                }
                var at = Math.atan2(m, g) * o, rt = t.resultTransform.skX;
                (S = (S - at) * A.radDeg + h - rt) > 180 ? S -= 360 : S < -180 && (S += 360), t.resultTransform.x = u, 
                t.resultTransform.y = _, t.resultTransform.skX = t.resultTransform.skY = rt + S * n, 
        ...
        _applyIk3(t, i, a, r, s, n) {
            ...
            this._sp.graphics.drawCircle(h, l, 15, "#ff00ff")), T = Math.atan2(l - t.resultMatrix.ty, h - t.resultMatrix.tx), 
            t.setRotation(T), (S = A._tempMatrix).identity(), S.rotate(T), S.scale(t.resultMatrix.getScaleX(), t.resultMatrix.getScaleY()), 
            S.translate(t.resultMatrix.tx, t.resultMatrix.ty), S.copyTo(t.resultMatrix), t.updateChild(), 
            ...
            S.copyTo(i.resultMatrix), i.updateChild();
        }
    }
    ...
    class S {
            ...
            if (this.target) {
                ...
                for (m = 0, I = 3; m < _; m++, I += 3) {
                    ...
                    if (A = f, T = M, r) {
                        ...
                        k = l ? D[I - 1] : 0 == d[m + 1] ? D[I + 2] : Math.atan2(F, C), k -= Math.atan2(E, U) - S / 180 * Math.PI, 
                        b && (P = Math.cos(k), L = Math.sin(k), A += ((g = y.length) * (P * U - L * E) - C) * a, 
                        T += (g * (L * U + P * E) - F) * a), k > Math.PI ? k -= 2 * Math.PI : k < -Math.PI && (k += 2 * Math.PI), 
        ...
        computeWorldVertices2(t, e, i, a, r, s) {
            ...
            if (null != o) {
                ...
                var b = T.tx, C = T.ty, F = T.a, w = T.b, k = T.c, P = T.d;
        ...
        computeWorldPositions(t, e, i, a, r, s, n) {
            ...
            var I = this._curves;
            l = 0;
            var A, T, S, b, C, F, w, k, P, L = h[0], U = h[1], B = 0, E = 0, R = 0, N = 0, O = 0, V = 0;
            for (x = 0, P = 2; x < D; x++, P += 6) C = 2 * (A = .1875 * (L - 2 * (B = h[P]) + (R = h[P + 2]))) + (S = .09375 * (3 * (B - R) - L + (O = h[P + 4]))), 
            F = 2 * (T = .1875 * (U - 2 * (E = h[P + 1]) + (N = h[P + 3]))) + (b = .09375 * (3 * (E - N) - U + (V = h[P + 5]))), 
            w = .75 * (B - L) + A + .16666667 * S, k = .75 * (E - U) + T + .16666667 * b, l += Math.sqrt(w * w + k * k), 
            w += C, k += F, C += S, F += b, l += Math.sqrt(w * w + k * k), w += C, k += F, l += Math.sqrt(w * w + k * k), 
            w += C + S, k += F + b, l += Math.sqrt(w * w + k * k), I[x] = l, L = O, U = V;
            ...
            for (x = 0, o = 0, u = 0, Y = 0; x < a; x++, o += 3) if ((_ = g += p = f[x]) < 0) this.addBeforePosition(_, h, 0, M, o); else if (_ > l) this.addAfterPosition(_ - l, h, y - 4, M, o); else {
                ...
                if (u != v) {
                    v = u;
                    var W = 6 * u;
                    for (C = 2 * (A = .03 * ((L = h[W]) - 2 * (B = h[W + 2]) + (R = h[W + 4]))) + (S = .006 * (3 * (B - R) - L + (O = h[W + 6]))), 
                    F = 2 * (T = .03 * ((U = h[W + 1]) - 2 * (E = h[W + 3]) + (N = h[W + 5]))) + (b = .006 * (3 * (E - N) - U + (V = h[W + 7]))), 
                    w = .3 * (B - L) + A + .16666667 * S, k = .3 * (E - U) + T + .16666667 * b, X = Math.sqrt(w * w + k * k), 
                    K[0] = X, W = 1; W < 8; W++) w += C, k += F, C += S, F += b, X += Math.sqrt(w * w + k * k), 
                    K[W] = X;
                    w += C, k += F, X += Math.sqrt(w * w + k * k), K[8] = X, w += C + S, k += F + b, 
    ...
    class F extends e.Sprite {
        ...
        init(t, i = 0) {
            ...
            a = t.pathArr.length; h < a; h++) r = t.pathArr[h], s = new S(r, this._boneList), 
            (n = this._boneSlotDic[r.name]) && ((s = new S(r, this._boneList)).target = n), 
        ...
        _createGraphics(t = -1) {
            ...
            g = this._ikArr.length; m < g; m++) (S = this._ikArr[m]).name in b && (S.bendDirection = b[S.name]), 
            S.name in C && (S.mix = C[S.name]), S.apply();
            if (this._pathDic) for (var F in this._pathDic) (T = this._pathDic[F]).apply(this._boneList, s);
        ...
        destroy(t = !0) {
            super.destroy(t), this._templet && this._templet._removeReference(1), this._templet = null, 
    ...
    class U extends _ {
        ...
        _parsePublicExtData() {
            ...
            var h, l, o = new e.Byte(this.getPublicExtData()), u = 0, _ = 0, p = 0, m = 0, x = 0, y = 0, A = 0, S = 0, C = 0, F = o.getInt32(), U = o.readUTFString(), B = U.split("\n");
            for (i = 0; i < F; i++) {
                ...
                C = o.getFloat32(), A = isNaN(C) ? p : C, C = o.getFloat32(), S = isNaN(C) ? m : C, 
                this.subTextureDic[U] = e.Texture.create(h, u, _, p, m, -x, -y, A, S);
            }
            this._mainTexture = h;
            ...
            for (i = 0; i < J; i++) {
                for (G = new T(), Z = o.getUint16(), a = 0; a < Z; a++) G.boneNames.push(o.readUTFString()), 
            ...
            var St = o.getInt16();
            if (St > 0) for (this.attachmentNames = [], i = 0; i < St; i++) this.attachmentNames.push(o.getUTFString());
    ...
    t.IkConstraintData = T, t.KeyFramesContent = s, t.MeshData = x, t.MovieClip = B, 
    t.PathConstraint = S, t.PathConstraintData = b, t.Skeleton = F, t.SkinData = w, 
