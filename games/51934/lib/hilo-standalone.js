...
(function(window) {
    ...
    var View = (function() {
        ...
        return Class.create( /** @lends View.prototype */ {
            ...
            /**
             * 设置背景填充样式。限制：DOMRenderer仅支持CSS颜色填充样式。
             * @param {Object} fillStyle 填充样式。可以是CSS颜色值、canvas的gradient或pattern填充。
             * @param {Number} x 背景填充的起始点的x轴坐标。
             * @param {Number} y 背景填充的起始点的y轴坐标。
             * @param {Number} width 背景填充的区域宽度。
             * @param {Number} height 背景填充的区域高度。
             * @returns 可视对象本身。
             */
     ...
     * @property {HTMLElement} canvas 舞台所对应的画布。它可以是一个canvas或一个普通的div。只读属性。
    ...
    var DOMElement = Class.create( /** @lends DOMElement.prototype */ {
        ...
        render: function(renderer, delta) {
            ...
            if (canvas.getContext) {
                var elem = this.drawable.domElement,
                    depth = this.depth,
                    nextElement = canvas.nextSibling,
                    nextDepth;
        ...
        return Class.create( /** @lends Graphics.prototype */ {
            ...
            cache: function(toImage) {
                var me = this,
                    cached = me._cache;
                if (!cached) {
                    cached = me._cache = Hilo.createElement('canvas', {
                        width: me.width,
                        height: me.height
                    });
