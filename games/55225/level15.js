...
var level15Init = [
	...
	[20,0,0,0,2,0,0,11,20],
	[20,0,0,9,0,7,0,1,20],
	[20,1,0,0,2,0,0,0,20],
	[20,0,0,7,12,2,9,0,20],
	[20,5,0,0,0,15,0,0,20],
	[20,0,1,0,3,0,0,1,20],
	[20,20,20,20,20,20,20,20,20]];
...
Level15.prototype = {
	...
	create: function(){
		...
		for(var i = 0; i < level15Init.length; i++){
			level[i] =[];
			for(var j = 0; j < level15Init[i].length; j++){
				level[i][j] = level15Init[i][j];
			}
		}		
	...
	goalBoard: function(){
		...
		if(keyNum == goal) {
			...
			var min = 29;// minimum moves to solution
			if (moveNum < min+1 && starsArray[14] < 3) starsArray[14] = 3;
			if (moveNum > min && moveNum < min+2 && starsArray[14] < 2) starsArray[14] = 2;
			if (moveNum > min+2 && starsArray[14] < 1) starsArray[14] = 1;
			console.log(moveNum);
...
if (typeof sdk !== 'undefined' && sdk.showBanner !== 'undefined') {sdk.showBanner()}
			console.log('LEVEL15 --> LEVEL16');
			//if(starsArray[15] == 4) starsArray[15] = 0;
			//localStorage.setItem(starsArrayLSkey, JSON.stringify(starsArray));
