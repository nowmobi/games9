...
Level12.prototype = {
	...
	create: function(){
		...
		for(var i = 0; i < level12Init.length; i++){
			level[i] =[];
			for(var j = 0; j < level12Init[i].length; j++){
				level[i][j] = level12Init[i][j];
			}
		}		
		...
		*/
		...
		var text1 = 'Find a safe path to the colored key.\n' +
			'Use the minimum amount of moves in order to obtain\n' +
			'greater score. The best score is 3 stars.';
	...
	goalBoard: function(){
		...
		if(keyNum == goal) {
			...
			var min = 27;// minimum moves to solution
			if (moveNum < min+1 && starsArray[11] < 3) starsArray[11] = 3;
			if (moveNum > min && moveNum < min+2 && starsArray[11] < 2) starsArray[11] = 2;
			if (moveNum > min+2 && starsArray[11] < 1) starsArray[11] = 1;
			console.log(moveNum);
...
if (typeof sdk !== 'undefined' && sdk.showBanner !== 'undefined') {sdk.showBanner()}
			console.log('LEVEL12 --> LEVEL13');
			if(starsArray[12] == 4) starsArray[12] = 0;
			localStorage.setItem(starsArrayLSkey, JSON.stringify(starsArray));
