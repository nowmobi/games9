...
var level06Init = [
	...
	[20,3,7,1,0,3,8,0,20],		
	[20,0,0,0,0,0,0,0,20],
	[20,1,0,10,2,0,0,0,20],
	[20,3,0,1,0,1,0,0,20],
	[20,0,5,0,0,0,6,0,20],
	[20,9,0,0,12,0,0,0,20],
	[20,1,0,0,3,0,0,3,20],
	[20,0,0,0,0,0,0,0,20],
	[20,20,20,20,20,20,20,20,20]];
...
Level06.prototype = {
	...
	init: function(){
		keyNum = 0; moveNum = 0;
		bobColor = 3;// 0=blue, 1=green, 2=orange, 3=purple, 4=red, 5=yellow, 6=white, 7=black (fixed set here)
		hintDir = 0;// directions are 0=Right,1=Up, 2=Left, 3=Down (fixed set here) 
		goal = 2;// number of keys to find (fixed set here)
	...
	create: function(){
		...
		for(var i = 0; i < level06Init.length; i++){
			level[i] =[];
			for(var j = 0; j < level06Init[i].length; j++){
				level[i][j] = level06Init[i][j];
			}
		}		
		...
		*/
		...
		var text1 = 'Paint Rollers sometimes must be thrown away to collect\n' +
	...
	goalBoard: function(){
		...
		if(keyNum == goal) {
			...
			if (moveNum < min+1 && starsArray[5] < 3) starsArray[5] = 3;
			if (moveNum > min && moveNum < min+2 && starsArray[5] < 2) starsArray[5] = 2;
			if (moveNum > min+2 && starsArray[5] < 1) starsArray[5] = 1;
			console.log(moveNum);
...
if (typeof sdk !== 'undefined' && sdk.showBanner !== 'undefined') {sdk.showBanner()}
			console.log('LEVEL06 --> LEVEL07');
			if(starsArray[6] == 4) starsArray[6] = 0;
			localStorage.setItem(starsArrayLSkey, JSON.stringify(starsArray));
