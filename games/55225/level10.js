...
var level10Init = [
	...
	[20,0,0,9,0,7,0,1,20],
	[20,1,0,0,2,0,0,0,20],
	[20,0,0,7,0,2,9,15,20],
	[20,5,0,0,0,0,0,3,20],
	[20,0,0,12,0,0,0,1,20],
	[20,20,20,20,20,20,20,20,20]];
...
Level10.prototype = {
	...
	create: function(){
		...
		for(var i = 0; i < level10Init.length; i++){
			level[i] =[];
			for(var j = 0; j < level10Init[i].length; j++){
				level[i][j] = level10Init[i][j];
			}
		}		
		...
		*/
		...
		var text1 = 'Find a safe path to the colored key.\n' +
			'Mixing primary colors makes the matching color of the key.';
	...
	goalBoard: function(){
		...
		if(keyNum == goal) {
			...
			if (moveNum < min+1 && starsArray[9] < 3) starsArray[9] = 3;
			if (moveNum > min && moveNum < min+2 && starsArray[9] < 2) starsArray[9] = 2;
			if (moveNum > min+2 && starsArray[9] < 1) starsArray[9] = 1;
			console.log(moveNum);
...
if (typeof sdk !== 'undefined' && sdk.showBanner !== 'undefined') {sdk.showBanner()}
			console.log('LEVEL10 --> LEVEL11');
			if(starsArray[10] == 4) starsArray[10] = 0;
			localStorage.setItem(starsArrayLSkey, JSON.stringify(starsArray));
