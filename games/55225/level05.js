...
Level05.prototype = {
	...
	init: function(){
		keyNum = 0; moveNum = 0;
		bobColor = 0;// 0=blue, 1=green, 2=orange, 3=purple, 4=red, 5=yellow, 6=white, 7=black (fixed set here)
		hintDir = 3;// directions are 0=Right,1=Up, 2=Left, 3=Down (fixed set here) 
		goal = 1;// number of keys to find (fixed set here)
	...
	create: function(){
		...
		for(var i = 0; i < level05Init.length; i++){
			level[i] =[];
			for(var j = 0; j < level05Init[i].length; j++){
				level[i][j] = level05Init[i][j];
			}
		}		
		...
		*/
		...
		var text1 = 'Watch out that Bob The Painter does not fall of the\n' +
		'building or into a floor hole.\n' +
		'Use the paint Rollers to stop Bob from falling.';
		...
		// --- functions -----------------------------------------
		function hint(){
			var hB = 3;// hint moving element index
	...
	goalBoard: function(){
		...
		if(keyNum == goal) {
			...
			var min = 11;// minimum moves to solution
			if (moveNum < min+1 && starsArray[4] < 3) starsArray[4] = 3;
			if (moveNum > min && moveNum < min+2 && starsArray[4] < 2) starsArray[4] = 2;
			if (moveNum > min+2 && starsArray[4] < 1) starsArray[4] = 1;
			console.log(moveNum);
...
if (typeof sdk !== 'undefined' && sdk.showBanner !== 'undefined') {sdk.showBanner()}
			console.log('LEVEL04 --> LEVEL06');
			if(starsArray[5] == 4) starsArray[5] = 0;
			localStorage.setItem(starsArrayLSkey, JSON.stringify(starsArray));
