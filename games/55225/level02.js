...
var level02Init = [
	...
	[20,5,0,0,0,0,0,0,20],
	[20,0,0,0,0,8,0,1,20],		
	[20,6,0,9,0,0,11,0,20],
	[20,0,0,0,0,0,2,0,20],
	[20,0,0,7,0,0,12,0,20],
	[20,1,0,0,0,0,2,0,20],
	[20,0,0,0,0,0,13,0,20],
	[20,0,0,1,3,0,1,0,20],
	[20,0,0,0,0,0,0,0,20],
	[20,20,20,20,20,20,20,20,20]];
...
Level02.prototype = {
	...
	init: function(){
		keyNum = 0; moveNum = 0;
		bobColor = 4;// 0=blue, 1=green, 2=orange, 3=purple, 4=red, 5=yellow, 6=white, 7=brown (fixed set here)
	...
	create: function(){
		...
		for(var i = 0; i < level02Init.length; i++){
			level[i] =[];
			for(var j = 0; j < level02Init[i].length; j++){
				level[i][j] = level02Init[i][j];
			}
		}		
		...
		*/
		...
		var text1 = 'Find a safe path to the colored key.\n' +
			'A key can be collected only if Bob The Painter and the key\n' +
			'have the same color.';
	...
	goalBoard: function(){
		...
		if(keyNum == goal) {
			...
			var min = 22;// minimum moves to solution
			if (moveNum < min+1 && starsArray[1] < 3) starsArray[1] = 3;
			if (moveNum > min && moveNum < min+2 && starsArray[1] < 2) starsArray[1] = 2;
			if (moveNum > min+2 && starsArray[1] < 1) starsArray[1] = 1;
			console.log(moveNum);
...
if (typeof sdk !== 'undefined' && sdk.showBanner !== 'undefined') {sdk.showBanner()}
			console.log('LEVEL02 --> LEVEL03');
			if(starsArray[2] == 4) starsArray[2] = 0;
			localStorage.setItem(starsArrayLSkey, JSON.stringify(starsArray));
