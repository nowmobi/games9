...
var level07Init = [
	...
	[20,0,0,0,0,11,3,0,20],
	[20,3,0,0,1,0,0,0,20],		
	[20,0,15,0,9,0,0,6,20],
	[20,0,0,14,1,0,1,0,20],
	[20,1,0,5,0,0,0,1,20],
	[20,0,3,1,0,0,2,0,20],
	[20,0,0,2,7,0,0,0,20],
	[20,3,0,0,0,0,0,8,20],
	[20,6,3,10,3,1,0,0,20],
	[20,20,20,20,20,20,20,20,20]];
...
Level07.prototype = {
	...
	init: function(){
		...
		hintDir = 2;// directions are 0=Right,1=Up, 2=Left, 3=Down (fixed set here) 
	...
	create: function(){
		...
		for(var i = 0; i < level07Init.length; i++){
			level[i] =[];
			for(var j = 0; j < level07Init[i].length; j++){
				level[i][j] = level07Init[i][j];
			}
		}		
		...
		*/
		...
		var text1 = 'If Bob The Painter turned brown, collect the can of white\n' +
			'paint to reset his color to white.';
	...
	goalBoard: function(){
		...
		if(keyNum == goal) {
			...
			var min = 28;// minimum moves to solution
			if (moveNum < min+1 && starsArray[6] < 3) starsArray[6] = 3;
			if (moveNum > min && moveNum < min+2 && starsArray[6] < 2) starsArray[6] = 2;
			if (moveNum > min+2 && starsArray[6] < 1) starsArray[6] = 1;
			console.log(moveNum);
...
if (typeof sdk !== 'undefined' && sdk.showBanner !== 'undefined') {sdk.showBanner()}
			console.log('LEVEL07 --> LEVEL08');
			if(starsArray[7] == 4) starsArray[7] = 0;
			localStorage.setItem(starsArrayLSkey, JSON.stringify(starsArray));
