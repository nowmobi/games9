...
var level09Init = [
	...
	[20,1,0,3,0,3,0,1,20],		
	[20,0,0,7,0,0,3,0,20],
	[20,3,0,10,0,12,6,0,20],
	[20,1,0,3,5,0,0,1,20],
	[20,0,0,13,0,15,0,0,20],
	[20,0,8,0,0,9,0,3,20],
	[20,1,2,0,3,0,2,1,20],
	[20,0,1,0,1,0,1,0,20],
	[20,20,20,20,20,20,20,20,20]];
...
Level09.prototype = {
	...
	init: function(){
		keyNum = 0; moveNum = 0;
		bobColor = 2;// 0=blue, 1=green, 2=orange, 3=purple, 4=red, 5=yellow, 6=white, 7=black (fixed set here)
	...
	create: function(){
		...
		for(var i = 0; i < level09Init.length; i++){
			level[i] =[];
			for(var j = 0; j < level09Init[i].length; j++){
				level[i][j] = level09Init[i][j];
			}
		}		
		...
		*/
		...
		var text1 = 'Pick up the can of white paint to reset Bob\'s color to\n' +
			'white. Avoid the floor holes.';
		...
		// --- functions -----------------------------------------
		function hint(){
			var hB = 5;// hint moving element index
	...
	goalBoard: function(){
		...
		if(keyNum == goal) {
			...
			if (moveNum < min+1 && starsArray[8] < 3) starsArray[8] = 3;
			if (moveNum > min && moveNum < min+2 && starsArray[8] < 2) starsArray[8] = 2;
			if (moveNum > min+2 && starsArray[8] < 1) starsArray[8] = 1;
			console.log(moveNum);
...
if (typeof sdk !== 'undefined' && sdk.showBanner !== 'undefined') {sdk.showBanner()}
			console.log('LEVEL09 --> LEVEL10');
			if(starsArray[9] == 4) starsArray[9] = 0;
			localStorage.setItem(starsArrayLSkey, JSON.stringify(starsArray));
