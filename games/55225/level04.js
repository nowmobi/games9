...
Level04.prototype = {
	...
	create: function(){
		...
		for(var i = 0; i < level04Init.length; i++){
			level[i] =[];
			for(var j = 0; j < level04Init[i].length; j++){
				level[i][j] = level04Init[i][j];
			}
		}		
	...
	goalBoard: function(){
		...
		if(keyNum == goal) {
			...
			var min = 6;// minimum moves to solution
			if (moveNum < min+1 && starsArray[3] < 3) starsArray[3] = 3;
			if (moveNum > min && moveNum < min+2 && starsArray[3] < 2) starsArray[3] = 2;
			if (moveNum > min+2 && starsArray[3] < 1) starsArray[3] = 1;
			console.log(moveNum);
...
if (typeof sdk !== 'undefined' && sdk.showBanner !== 'undefined') {sdk.showBanner()}
			console.log('LEVEL04 --> LEVEL05');
			if(starsArray[4] == 4) starsArray[4] = 0;
			localStorage.setItem(starsArrayLSkey, JSON.stringify(starsArray));
