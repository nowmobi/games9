...
Level03.prototype = {
	...
	init: function(){
		keyNum = 0; moveNum = 0;
		bobColor = 3;// 0=blue, 1=green, 2=orange, 3=purple, 4=red, 5=yellow, 6=white, 7=brown (fixed set here)
		hintDir = 1;// directions are 0=Right,1=Up, 2=Left, 3=Down (fixed set here) 
	...
	create: function(){
		...
		for(var i = 0; i < level03Init.length; i++){
			level[i] =[];
			for(var j = 0; j < level03Init[i].length; j++){
				level[i][j] = level03Init[i][j];
			}
		}		
		...
		*/
		...
		var text1 = 'Sometimes Bob The painter must mix different colors to\n' +
			'match the color of the keys.\n' + 
			'Do not let Bob fall of the floor.';
		...
		// --- functions -----------------------------------------
		function hint(){
			var hB = 0;// hint moving element index
	...
	goalBoard: function(){
		...
		if(keyNum == goal) {
			...
			var min = 9;// minimum moves to solution
			if (moveNum < min+1 && starsArray[2] < 3) starsArray[2] = 3;
			if (moveNum > min && moveNum < min+2 && starsArray[2] < 2) starsArray[2] = 2;
			if (moveNum > min+2 && starsArray[2] < 1) starsArray[2] = 1;
			console.log(moveNum);
...
if (typeof sdk !== 'undefined' && sdk.showBanner !== 'undefined') {sdk.showBanner()}
			console.log('LEVEL03 --> LEVEL04');
			if(starsArray[3] == 4) starsArray[3] = 0;
			localStorage.setItem(starsArrayLSkey, JSON.stringify(starsArray));
