...
var level08Init = [
	...
	[20,6,0,0,7,0,0,5,20],
	[20,0,0,0,8,0,1,0,20],		
	[20,6,12,0,0,9,15,2,20],
	[20,0,0,1,3,0,1,0,20],
	[20,0,1,13,0,0,0,11,20],
	[20,0,2,0,0,7,0,0,20],
	[20,0,1,0,0,6,0,3,20],
	[20,0,0,1,0,0,3,0,20],
	[20,6,0,0,6,8,0,9,20],
...
Level08.prototype = {
	...
	create: function(){
		...
		for(var i = 0; i < level08Init.length; i++){
			level[i] =[];
			for(var j = 0; j < level08Init[i].length; j++){
				level[i][j] = level08Init[i][j];
			}
		}		
		...
		*/
		...
		var text1 = 'Find a safe path to the colored key.\n' +
			'Use the Paint Cans to get the matching color of the key\n' +
			'in order to collect the key.';
	...
	goalBoard: function(){
		...
		if(keyNum == goal) {
			...
			if (moveNum < min+1 && starsArray[7] < 3) starsArray[7] = 3;
			if (moveNum > min && moveNum < min+2 && starsArray[7] < 2) starsArray[7] = 2;
			if (moveNum > min+2 && starsArray[7] < 1) starsArray[7] = 1;
			console.log(moveNum);
...
if (typeof sdk !== 'undefined' && sdk.showBanner !== 'undefined') {sdk.showBanner()}
			console.log('LEVEL08 --> LEVEL09');
			if(starsArray[8] == 4) starsArray[8] = 0;
			localStorage.setItem(starsArrayLSkey, JSON.stringify(starsArray));
