...
var level13Init = [
	...
	[20,0,0,0,0,1,0,0,20],
	[20,1,3,0,3,0,0,1,20],		
	[20,3,0,0,0,7,0,13,20],
	[20,1,6,0,1,0,0,8,20],
	[20,0,0,9,0,14,0,3,20],
	[20,0,0,0,0,0,0,0,20],
	[20,5,0,0,0,0,0,0,20],
	[20,1,0,3,0,0,0,1,20],
	[20,0,0,1,3,11,0,0,20],
	[20,20,20,20,20,20,20,20,20]];
...
Level13.prototype = {
	...
	create: function(){
		...
		for(var i = 0; i < level13Init.length; i++){
			level[i] =[];
			for(var j = 0; j < level13Init[i].length; j++){
				level[i][j] = level13Init[i][j];
			}
		}		
		...
		*/
		...
		// --- functions -----------------------------------------
		function hint(){
			var hB = 4;// hint moving element index
	...
	goalBoard: function(){
		...
		if(keyNum == goal) {
			...
			var min = 37;// minimum moves to solution
			if (moveNum < min+1 && starsArray[12] < 3) starsArray[12] = 3;
			if (moveNum > min && moveNum < min+2 && starsArray[12] < 2) starsArray[12] = 2;
			if (moveNum > min+2 && starsArray[12] < 1) starsArray[12] = 1;
			console.log(moveNum);
...
if (typeof sdk !== 'undefined' && sdk.showBanner !== 'undefined') {sdk.showBanner()}
			console.log('LEVEL13 --> LEVEL14');
			if(starsArray[13] == 4) starsArray[13] = 0;
			localStorage.setItem(starsArrayLSkey, JSON.stringify(starsArray));
