...
var level11Init = [
	...
	[20,0,9,0,0,0,2,0,20],
	[20,1,0,0,3,0,0,7,20],
	[20,1,0,0,0,12,0,1,20],
	[20,6,13,1,2,1,0,0,20],
	[20,11,0,0,0,0,8,0,20],
	[20,1,0,0,3,0,0,0,20],
	[20,20,20,20,20,20,20,20,20]];
...
Level11.prototype = {
	...
	create: function(){
		...
		for(var i = 0; i < level11Init.length; i++){
			level[i] =[];
			for(var j = 0; j < level11Init[i].length; j++){
				level[i][j] = level11Init[i][j];
			}
		}		
		...
		*/
		...
		var text1 = 'Find a safe path to the colored key.\n' +
			'The order in which you collect the colored keys matters.';
	...
	goalBoard: function(){
		...
		if(keyNum == goal) {
			...
			var min = 33;// minimum moves to solution
			if (moveNum < min+1 && starsArray[10] < 3) starsArray[10] = 3;
			if (moveNum > min && moveNum < min+2 && starsArray[10] < 2) starsArray[10] = 2;
			if (moveNum > min+2 && starsArray[10] < 1) starsArray[10] = 1;
			console.log(moveNum);
...
if (typeof sdk !== 'undefined' && sdk.showBanner !== 'undefined') {sdk.showBanner()}
			console.log('LEVEL11 --> LEVEL12');
			if(starsArray[11] == 4) starsArray[11] = 0;
			localStorage.setItem(starsArrayLSkey, JSON.stringify(starsArray));
