...
var level01Init = [
	...
	[20,0,0,0,0,8,0,1,20],		
	[20,0,3,0,0,0,0,0,20],		
	[20,1,5,0,6,0,10,0,20],
	[20,0,0,0,0,0,2,0,20],
	[20,0,0,0,7,0,14,0,20],
	[20,1,1,0,0,0,2,0,20],
	[20,6,0,0,9,0,15,0,20],
	[20,0,0,1,0,0,0,1,20],
	[20,0,0,0,0,0,0,0,20],
	[20,20,20,20,20,20,20,20,20]];
...
Level01.prototype = {
	...
	init: function(){
		keyNum = 0; moveNum = 0;
		bobColor = 1;// 0=blue, 1=green, 2=orange, 3=purple, 4=red, 5=yellow, 6=white, 7=brown (fixed set here)
		hintDir = 0;// directions are 0=Right,1=Up, 2=Left, 3=Down (fixed set here) 
		goal = 3;// number of keys to find (fixed set here)
	...
	create: function(){
		...
		for(var i = 0; i < level01Init.length; i++){
			level[i] =[];
			for(var j = 0; j < level01Init[i].length; j++){
				level[i][j] = level01Init[i][j];
			}
		}		
		...
		*/
		...
		var text1 = 'Click (Tap) on Bob The Painter and select an arrow to let him\n' +
			'move towards the colored key. The number of required keys\n' +
			'to collect is indicated at the top board.';
		...
		// --- functions -----------------------------------------
		function hint(){
			var hB = 1;// hint moving element index
	...
	goalBoard: function(){
		...
		if(keyNum == goal) {
			...
			var min = 13;// minimum moves to solution
			if (moveNum < min+1 && starsArray[0] < 3) starsArray[0] = 3;
			if (moveNum > min && moveNum < min+2 && starsArray[0] < 2) starsArray[0] = 2;
			if (moveNum > min+2 && starsArray[0] < 1) starsArray[0] = 1;
			console.log(moveNum);
...
if (typeof sdk !== 'undefined' && sdk.showBanner !== 'undefined') {sdk.showBanner()}
			console.log('LEVEL01 --> LEVEL02');
			if(starsArray[1] == 4) starsArray[1] = 0;
			localStorage.setItem(starsArrayLSkey, JSON.stringify(starsArray));
